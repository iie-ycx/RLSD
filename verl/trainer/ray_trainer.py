...
class RayPPOTrainer:
    ...
    def _balance_batch(self, batch: DataProto, metrics: dict[str, Any], logging_prefix: str = "global_seqlen") -> torch.Tensor:
    ...
    def _make_batch_data(self, metrics: dict[str, Any], reward_futures: list = None) -> DataProto:
        batch = None
        ...
        while True:
            ...
            # pipeline reward: submit async reward computation right after each mini-batch rollout
            if (
                reward_futures is not None
                and self.config.algorithm.pipeline_reward
                and not self.config.algorithm.online_filtering
            ):
                mini_reward_batch = new_batch.select(
                    batch_keys=["responses", "response_mask"],
                    non_tensor_batch_keys=[k for k in new_batch.non_tensor_batch if k != "multi_modal_data"],
                )
                reward_ref = self.reward_fn.compute_reward.remote(mini_reward_batch)
                reward_futures.append((reward_ref, len(new_batch)))
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    if reward_futures and len(reward_futures) > 0:
                        # pipeline reward: collect all async reward futures, concat, truncate, reorder
                        with timer("reward", timing_raw):
                            all_reward_tensors = []
                            all_reward_metrics = defaultdict(list)
                            for ref, mini_size in reward_futures:
                            ...
                            # concat along batch dim, then truncate to actual batch size
                            ...
                            # reorder to match balanced batch
                            reward_tensor_all = reward_tensor_all[global_idx]
                            batch.batch["token_level_scores"] = reward_tensor_all
                            reward_metrics = {f"reward/{k}": v for k, v in reduce_metrics(all_reward_metrics).items()}
